 ...
 * Form for authenticated users to report lost items
 */
...
function ReportLostContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rawDate = formData.date_lost;
      ...
      uploadData.append('date_lost', dateToSubmit);
      uploadData.append('file', formData.image);

      // Upload to backend
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/items/lost`,
        uploadData,
        {
          headers: {
      ...
      if (response.status === 201) {
        setSuccess('Lost item reported successfully! We will notify you if someone finds it.');
        setFormData({
          description: '',
          location: '',
          date_lost: '',
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        ...
        setError(err.message || 'Failed to report lost item. Check your connection.');
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="bg-white rounded-lg shadow-md p-8">
            ...
            <div className="mt-6 pt-6 border-t">
              ...
              <ul className="text-sm text-gray-600 space-y-2">
                ...
                <li>✓ Be as accurate as possible with the date and time</li>
              </ul>
            </div>
          </div>
        </div>
